...
import { loadTruyenUrl } from '@/lib/appConfig';

const TRUYEN_URL = 'https://truyenfullmoi.com/';
const TRUYEN_LOGO = 'https://res.cloudinary.com/df2amyjzw/image/upload/v1775745095/ganhgame_g8zdu4.png';
...
export default function GanhTruyenScreen() {
	const router = useRouter();
	const webViewRef = useRef<WebView>(null);
	const [canGoBack, setCanGoBack] = useState(false);
	const [loading, setLoading] = useState(true);
	const [truyenUrl, setTruyenUrl] = useState<string | null>(null);
	...
	useEffect(() => {
		...
		(async () => {
			const url = await loadTruyenUrl(TRUYEN_URL);
			if (!cancelled) {
				setTruyenUrl(url);
			}
		})();
	...
	return (
		<SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
			<View style={styles.header}>
				...
				<Image source={{ uri: TRUYEN_LOGO }} style={styles.headerLogo} contentFit="contain" />
